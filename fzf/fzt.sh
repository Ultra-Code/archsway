...
FZF_DEFAULT_COMMAND="find $path  -name .git -prune -o -type f -print" \
  ...
  --bind "ctrl-f:reload(find $path -name .git -prune -o -type f -print)+change-prompt(file> )+change-preview(bat --style=numbers --color=always --line-range :500 {}),enter:execute:hx {}" \
